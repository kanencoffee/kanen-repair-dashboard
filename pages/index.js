...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const summaryRes = await fetch('http://localhost:8000/v1/dashboard/summary');
      const summaryData = await summaryRes.json();
      setSummary(summaryData);

      const stockoutRes = await fetch('http://localhost:8000/v1/analytics/stockout-alerts');
      const stockoutData = await stockoutRes.json();
      setStockoutAlerts(stockoutData);

      const velocityRes = await fetch('http://localhost:8000/v1/analytics/repair-velocity');
      const velocityData = await velocityRes.json();
      setRepairVelocity(velocityData);

      const techRes = await fetch('http://localhost:8000/v1/analytics/brand-profiles');
      const techData = await techRes.json();
      setTechPerformance(techData);

      const brandRes = await fetch('http://localhost:8000/v1/analytics/brand-profiles');
      const brandData = await brandRes.json();
      setBrandProfiles(brandData);

      const profitRes = await fetch('http://localhost:8000/v1/analytics/parts-profit');
      const profitData = await profitRes.json();
      setPartsProfit(profitData);

      const burnRes = await fetch('http://localhost:8000/v1/analytics/burn-rates');
      const burnData = await burnRes.json();
      setBurnRates(burnData);

      const repeatRes = await fetch('http://localhost:8000/v1/analytics/repeat-customers');
